...
namespace Spark2Scale_.Server.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
            ...
            try
            {
                ...
                switch (req.UserType.ToLower())
                {
                    case "founder":
                        ...
                        break;
                    case "investor":
                        // FIX: Ensure tags is compatible with string[]
                        var tagsArray = req.Tags ?? new string[0];
        ...
        public async Task<IActionResult> SignIn([FromBody] SignInRequest req)
        {
            try
            {
                ...
                if (auth.User.UserMetadata != null && auth.User.UserMetadata.ContainsKey("user_type"))
                {
                ...
                return Ok(new
                {
                    ...
                    {
                        id = uid,
                        email = db?.email ?? req.Email,
                        name = db != null ? $"{db.fname} {db.lname}" : "",
    ...
    public class SignUpRequest
    {
        ...
        // FIX: Changed List<string> to string[] to match database model
