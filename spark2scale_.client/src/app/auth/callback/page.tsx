    ...
    const handleVerification = async () => {
      try {
        ...
        if (
          ...
          console.log(
            `[AuthCallback] Warning: Unexpected verification type '${type}'. Proceeding as it includes an access token.`,
          );
        }

        const apiUrl =
          process.env.NEXT_PUBLIC_API_URL || "http://localhost:5231";
        ...
        if (!type && accessToken) {
          ...
          if (signupCtxStr) {
            try {
              const ctx = JSON.parse(signupCtxStr) as {
                userType?: string;
                tags?: string[];
              };
        ...
        if (data.token && typeof data.token === "string") {
          ...
          const postVerificationInviteStr = localStorage.getItem(
            "postVerificationInvitation",
          );
          ...
          if (postVerificationInviteStr) {
            try {
              ...
              console.log(
                "Found post-verification invitation context in LocalStorage:",
                inviteCtx,
              );
            } catch (e) {
          ...
          if (!inviteCtx && userMetadata?.startup_id) {
            console.log(
              "LocalStorage empty. Falling back to User Metadata for Startup linking.",
            );
            inviteCtx = {
          ...
          if (inviteCtx?.startupId) {
            try {
              ...
              // We need the User ID
              const userId =
                data.user?.uid || data.user?.id || (data.user as any)?.UserId;

              if (userId) {
                console.log(
                  "Attempting to atomically accept invitation for user:",
                  userId,
                );
                ...
                if (inviteCtx.token) {
                  ...
                  console.log(
                    "No token available, relying on Backend VerifyEmail linking or previous association.",
                  );
                }

                console.log(
                  "Atomic handshake successful (or assumed by Backend)!",
                );
          ...
          console.log(
            "[AuthCallback] Resolved userType from server:",
            resolvedUserType,
          );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#F0EADC] via-[#fff] to-[#FFD95D]/20">
        <div className="text-center space-y-4">
          ...
          <p className="text-lg font-semibold text-[#576238]">
